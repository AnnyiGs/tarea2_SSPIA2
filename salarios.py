...
FEATURE_COLS = NUM_COLS + CAT_COLS   # orden fijo

X_raw = df[FEATURE_COLS].copy()
y     = df[TARGET_COL].values.astype(float)

# --- LabelEncoding para categóricas ---
label_encoders = {}
for col in CAT_COLS:
    le = LabelEncoder()
...
def build_mlp(n_features, hidden_layers=2, neurons=64,
    ...
    opt = opt_map.get(optimizer.lower(),
    ...
    model.add(layers.Dense(1))  # Salida: regresion continua
...
def train_and_evaluate(model, X_tr, y_tr, X_te, y_te,
                       epochs=200, batch_size=32, label=""):
    """Entrena y devuelve historial + metricas en escala original."""
    ...
    print(f"  [{label:<22}] Epocas: {len(history.history['loss']):3d} | "
          f"RMSE: ${rmse:>9,.2f} | MAE: ${mae:>9,.2f} | R2: {r2:.4f}")
...
OPT_COLORS = {"adam": "#2563EB", "sgd": "#DC2626",
              "rmsprop": "#16A34A", "adadelta": "#D97706"}

opt_results = {}

for opt_name in OPTIMIZERS:
    model = build_mlp(n_features, hidden_layers=2, neurons=64,
                      optimizer=opt_name, learning_rate=0.001)
    hist, rmse, mae, r2, y_pred, y_real = train_and_evaluate(
        model, X_train, y_train, X_test, y_test,
        epochs=200, batch_size=32, label=opt_name.upper()
    )
    opt_results[opt_name] = dict(history=hist, rmse=rmse, mae=mae,
                                 r2=r2, y_pred=y_pred, y_real=y_real)

# --- Figura: curvas de perdida por optimizador ---
...
for ax, opt in zip(axes.flatten(), OPTIMIZERS):
    h    = opt_results[opt]["history"]
    r2   = opt_results[opt]["r2"]
    rmse = opt_results[opt]["rmse"]
    col  = OPT_COLORS[opt]
    ax.plot(h.history["loss"],     label="Entrenamiento", color=col, linewidth=2)
    ax.plot(h.history["val_loss"], label="Validacion",    color=col,
            linewidth=2, linestyle="--", alpha=0.6)
    ax.set_title(f"Optimizador: {opt.upper()}\n"
                 f"R2 = {r2:.4f}   RMSE = ${rmse:,.0f}", fontsize=11)
...
plt.show()
print("\n✔ Figura guardada: DATASET-SALARY/fig_optimizadores.png")
...
# (hidden_layers, neurons, learning_rate, epochs, batch_size)
CONFIGS = [
    (1,  32,  0.001,  200, 32),
    (1,  64,  0.001,  200, 32),
    (2,  64,  0.001,  200, 32),
...
for cfg in CONFIGS:
    ...
    model = build_mlp(n_features, hidden_layers=hl, neurons=neu,
                      optimizer=best_opt, learning_rate=lr)
    hist, rmse, mae, r2, _, _ = train_and_evaluate(
        model, X_train, y_train, X_test, y_test,
        epochs=ep, batch_size=bs, label=lbl
    )
    hp_records.append({
        "Capas Ocultas":  hl,
        "Neuronas/Capa":  neu,
        "Learning Rate":  lr,
        "Batch Size":     bs,
        "Epocas Reales":  len(hist.history["loss"]),
        "RMSE ($)":       round(rmse, 2),
        "MAE ($)":        round(mae,  2),
        "R2 (Accuracy)":  round(r2,   4)
    })
...
print("\n✔ Tabla guardada: DATASET-SALARY/tabla_hiperparametros.csv")

# --- Figura: R2 por configuracion ---
best_idx   = hp_df["R2 (Accuracy)"].idxmax()
colors_bar = ["#2563EB" if i == best_idx else "#93C5FD"
              for i in range(len(hp_records))]
x_labels   = [
    f"L={r['Capas Ocultas']} N={r['Neuronas/Capa']}\nlr={r['Learning Rate']} bs={r['Batch Size']}"
    for r in hp_records
]

fig2, ax = plt.subplots(figsize=(15, 5))
bars = ax.bar(range(len(hp_records)), hp_df["R2 (Accuracy)"],
              color=colors_bar, edgecolor="white", linewidth=0.8)
ax.set_xticks(range(len(hp_records)))
ax.set_xticklabels(x_labels, fontsize=8)
ax.set_ylabel("R2 (Overall Accuracy)")
ax.set_title(f"Comparacion de Hiperparametros — R2 en Prueba\n"
             f"(Optimizador: {best_opt.upper()})",
             fontsize=12, fontweight="bold")
ax.set_ylim(max(0, hp_df["R2 (Accuracy)"].min() - 0.05), 1.0)
ax.axhline(hp_df["R2 (Accuracy)"].max(), color="#DC2626",
           linestyle="--", linewidth=1, alpha=0.7, label="Mejor R2")
...
plt.show()
print("✔ Figura guardada: DATASET-SALARY/fig_hiperparametros.png")

# --- Curvas de perdida de las 10 configuraciones ---
fig3, axes3 = plt.subplots(2, 5, figsize=(20, 7))
fig3.suptitle(f"Curvas de Perdida — Ajuste de Hiperparametros ({best_opt.upper()})",
...
for ax, (lbl, hist), r2 in zip(axes3.flatten(), hp_histories, hp_df["R2 (Accuracy)"]):
    ...
    ax.plot(hist.history["val_loss"], color="#DC2626", linewidth=1.5,
            linestyle="--", label="Val")
    ax.set_title(f"{lbl}\nR2={r2:.4f}", fontsize=8)
...
plt.show()
print("✔ Figura guardada: DATASET-SALARY/fig_curvas_hp.png")
...
hl_b, neu_b, lr_b, ep_b, bs_b = best_cfg
print(f"\n✔ Mejor configuracion encontrada:")
print(f"  Capas ocultas : {hl_b}")
print(f"  Neuronas/capa : {neu_b}")
print(f"  Learning rate : {lr_b}")
print(f"  Batch size    : {bs_b}")
print(f"  Optimizador   : {best_opt.upper()}")

final_model = build_mlp(n_features, hidden_layers=hl_b, neurons=neu_b,
                        optimizer=best_opt, learning_rate=lr_b)
es_final = EarlyStopping(monitor="val_loss", patience=25,
                         restore_best_weights=True, verbose=0)
...
ss_res   = np.sum((y_real_fin - y_pred_fin) ** 2)
ss_tot   = np.sum((y_real_fin - np.mean(y_real_fin)) ** 2)
r2_fin   = 1 - ss_res / ss_tot

print(f"\n  RMSE Final : ${rmse_fin:,.2f}")
print(f"  MAE Final  : ${mae_fin:,.2f}")
print(f"  R2 Final   : {r2_fin:.4f}")

# --- Figura evaluacion final ---
fig4, (ax4a, ax4b) = plt.subplots(1, 2, figsize=(14, 5))
fig4.suptitle(f"Evaluacion del Modelo Final  ({best_opt.upper()} · "
              f"L={hl_b} · N={neu_b} · lr={lr_b} · bs={bs_b})",
              fontsize=13, fontweight="bold")

ax4a.plot(hist_final.history["loss"],     color="#2563EB", linewidth=2, label="Entrenamiento")
ax4a.plot(hist_final.history["val_loss"], color="#DC2626", linewidth=2,
...
lims = [min(y_real_fin.min(), y_pred_fin.min()) * 0.95,
        max(y_real_fin.max(), y_pred_fin.max()) * 1.05]
...
plt.show()
print("✔ Figura guardada: DATASET-SALARY/fig_evaluacion_final.png")
...
X_new_arr    = X_new.values.astype(float)
X_new_scaled = scaler_X.transform(X_new_arr)

y_new_sc   = final_model.predict(X_new_scaled, verbose=0).ravel()
y_new_pred = scaler_y.inverse_transform(y_new_sc.reshape(-1, 1)).ravel()
...
for i, (pred, des) in enumerate(zip(y_new_pred, desired_salaries)):
    ...
    result_rows.append({
        "Empleado":             i + 1,
        "Salario Predicho ($)": round(pred, 2),
        "Salario Deseado ($)":  round(des,  2),
        "Diferencia ($)":       round(pred - des, 2),
...
print("\n✔ Tabla guardada: DATASET-SALARY/predicciones_nuevos.csv")

# --- Figura: predicho vs deseado ---
...
fig5.suptitle("Predicciones vs Salario Deseado — 5 Nuevos Empleados",
              fontsize=13, fontweight="bold")

bars1 = ax5a.bar(x_pos - width/2, y_new_pred,       width,
                 label="Predicho", color="#2563EB")
bars2 = ax5a.bar(x_pos + width/2, desired_salaries, width,
                 label="Deseado",  color="#DC2626")
...
for i, val in enumerate(diff_pcts):
    offset = 0.4 if val >= 0 else -1.5
    ax5b.text(i, val + offset, f"{val:+.2f}%",
              ha="center", va="bottom", fontsize=9, fontweight="bold")

plt.tight_layout()
plt.savefig("DATASET-SALARY/fig_predicciones_nuevos.png", dpi=150, bbox_inches="tight")
plt.show()
print("✔ Figura guardada: DATASET-SALARY/fig_predicciones_nuevos.png")


print("\n" + "=" * 65)
print("  7. RESUMEN FINAL Y CONCLUSIONES")
print("=" * 65)
...
print(f"  RMSE : ${rmse_fin:,.2f}")
print(f"  MAE  : ${mae_fin:,.2f}")
print(f"  R2   : {r2_fin:.4f}")

print("\n--- Resultados 5 Nuevos Empleados ---")
print(results_df.to_string(index=False))


print("=" * 65)
print("  Script finalizado.")
print("  Archivos generados en DATASET-SALARY/:")
print("    fig_optimizadores.png")
print("    fig_hiperparametros.png")
print("    fig_curvas_hp.png")
print("    fig_evaluacion_final.png")
print("    fig_predicciones_nuevos.png")
print("    tabla_hiperparametros.csv")
print("    predicciones_nuevos.csv")
print("=" * 65)